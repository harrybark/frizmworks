...
<div class="findId_s1">
	...
	<ul>
		<li><input type="text" name="memberId" class="memberId"
			id="memberId" placeholder="아이디를 입력하십시오."></li>
		...
		<li>
			<button class="findIdBtn1">비밀번호 찾기</button>
		</li>
	</ul>
</div>
<script>
			...
			.click(
					function() {
						...
						var dataString = "memberId=" + memberId
								+ "&memberEmail=" + memberEmail + "&emailBox="
								+ emailBox;
						// 4자리 ~ 12자리의 대소문자와 숫자가 결합된 것
						var regId = /^[A-za-z0-9]{4,12}$/g;
						//이메일 유효성 검사
						var regExp1 = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*/i;
						var regExp2 = /^[0-9a-zA-Z][0-9a-zA-Z\_\-]*[0-9a-zA-Z](\.[a-zA-Z]{2,6}){1,2}$/i;

						if (!regId.test(memberId)) {
							alert('아이디는 4~12자리의 대소문자와 숫자가 결합되어 있습니다.');
							$btn2.find('.memberId2').focus();
							return false;
						}
...
<div class="findId_s2">
	...
	<form action="" method="post" name="pwform2">
		<ul>
			<li><input type="text" name="memberId" class="memberId2"
				placeholder="아이디를 입력하십시오."></li>
			<li>
...
<script>
	...
	$('.findIdBtn2')
			.click(
					function() {
						var $btn2 = $(this).parents('form[name=pwform2]');
						var memberId = $btn2.find('.memberId2').val();
						var mobile2 = $btn2.find('#mobile2').val();
						var mobile2_1 = $btn2.find('#mobile2_1').val();
						var mobile2_2 = $btn2.find('#mobile2_2').val();
						console.log(memberId);
						console.log(mobile2);
						var dataString = "memberId=" + memberId + "&mobile2="
								+ mobile2 + "&mobile2_1=" + mobile2_1
								+ "&mobile2_2=" + mobile2_2;
						...
						if (!regexNum.test(mobile2_1)
								|| !regexNum.test(mobile2_2)) {
							alert('전화번호는 숫자만 입력 가능합니다.');
							$btn2.find('#mobile2_1').focus();
							return false;
						} else {
							$.ajax({
								url : "findPwUsingMobile.mo",
								data : dataString,
								type : "post",
								success : function(data) {
									if (!$.trim(data) || !data || data == ''
